...
if ($ExecutionContext.SessionState.LanguageMode -ne "FullLanguage") {
    Write-Host "Error: HideMe is unable to run on your system, powershell execution is restricted by security policies" -ForegroundColor Red
    Write-Output ""
...
function ShowAppSelectionForm {
    ...
    $form.Text = "HideMe Application Selection"
...
if ((Get-AppxPackage -Name "*Microsoft.DesktopAppInstaller*") -and ([int](((winget -v) -replace 'v','').split('.')[0..1] -join '') -gt 14)) {
    ...
    if (-not $Silent) {
        Write-Warning "Winget is not installed or outdated. This may prevent HideMe from removing certain apps."
        Write-Output ""HideMe
...
# Hide progress bars for app removal, as they block HideMe's output
...
if ($global:Params.ContainsKey("Sysprep")) {
    ...
    if (-not (Test-Path "$defaultUserPath")) {
        Write-Host "Error: Unable to start HideMe in Sysprep mode, cannot find default user folder at '$defaultUserPath'" -ForegroundColor Red
        AwaitKeyToExit
        Exit
    }
    # Exit script if run in Sysprep mode on Windows 10
    if ($WinVersion -lt 22000) {
        Write-Host "Error: HideMe Sysprep mode is not supported on Windows 10" -ForegroundColor Red
        AwaitKeyToExit
        Exit
    }
}
...
if ($global:Params.ContainsKey("User")) {
    ...
    if (-not (Test-Path "$userPath")) {
        Write-Host "Error: Unable to run HideMe for user $($global:Params.Item("User")), cannot find user data at '$userPath'" -ForegroundColor Red
        AwaitKeyToExit
        Exit
    }
}
...
if ((-not $global:Params.Count) -or $RunDefaults -or $RunWin11Defaults -or $RunSavedSettings -or ($SPParamCount -eq $global:Params.Count)) {
    ...
    switch ($Mode) {
        ...
        '4' {
            PrintHeader 'Custom Mode'
            Write-Output "HideMe will make the following changes:"
